...
__all__ = ['QAOA', 'SUPPORTED_TASKS', 'QAOA_LOCAL', 'QAOA', 'SUPPORTED_TASKS', 'QAOA_LOCAL']
...
class QAOA(QuantumOptimizer):
    ...
    * **params**  : user-supplied initialization of params.  If None, a default random initialization is used.
    ...
    def __init__(self, g, task=None, n_layers=4, constrained=False,
                 device_name=DEFAULT_LOCAL_SIMULATOR, device_arn=None, s3_folder=None,
                 params=None):
        """
        constructor
        """
        device_name = QAOA_LOCAL if device_name==DEFAULT_LOCAL_SIMULATOR else device_name
        # error checks
        if task not in SUPPORTED_TASKS: raise ValueError(f'task {task} is not supported. ' +\
                                                         f'Supported tasks: {SUPPORTED_TASKS}')
        if device_name != QAOA_LOCAL and (device_arn is None or s3_folder is None):
    ...
    def _device(self, shots=None):
        kwargs = {}
        if self.device_name != QAOA_LOCAL:
